...
The minimum for $\gamma_{jm}$ is the sum of the residuals, $\gamma_{jm} = \sum_{x_i \in R_{jm}} (p-y_i)$. Meaning we should adjust the output at each terminal node by the sum of the difference between the observed value and predicted probability $p(x_i)$ for each point $x_i$ belonging to that terminal node.

\newpage
b) The general form of the 2nd order Taylor polynomial of the loss function is going to be:
...
For the MSE, the 2nd derivative will be a constant and thus the optimal weights will again be $\hat{\gamma_{jm}} = \frac{\sum_{x_i \in R_{jm}} (y_i - f_{m-1}(x_i))}{n_{jm}}$. This is because the term $\frac{h^2}{2!}f''(x) = \frac{- n_{jm} \gamma^2}{2!}$ will vanish when we take the first derivative. We show below that indeed the 2nd derivative is equal to $- n_{jm}$ for MSE loss function.
...
\frac{d^2}{d^2\gamma_{jm}} L(y_i, f_{m-1}(x_i) + \gamma_{jm}) &= - n_{jm} < 0 \text{ } \forall \ x_i
\end{align}
```
