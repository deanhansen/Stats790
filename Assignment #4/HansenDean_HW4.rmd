...
0 &= \sum_{x_i \in R_{jm}} (y_i - p)
\end{align}
```

The minimum for $\gamma_{jm}$ is the sum of the residuals, $\gamma_{jm} = \sum_{x_i \in R_{jm}} (y_i - p)$.
...
For the MSE, the 2nd derivative will be a constant and thus the optimal weights will again be $\gamma_{jm} = \frac{\sum_{x_i \in R_{jm}} (y_i - f_{m-1}(x_i))}{n_{jm}}$. This is because the term $\frac{h^2}{2!}f''(x) = \frac{- n_{jm} \gamma^2}{2!}$ will vanish when we take the first derivative. We show below that indeed the 2nd derivative is equal to $- n_{jm}$ for MSE loss function.
...
For the binomial log-likelihood, we need to find the 2nd order Taylor polynomial of our loss function and then differentiate this with respect to gamma. To do this, we need to calculate the 2nd derivative of the loss function in part a). We drop the sum here for convenience. 

```{=latex}
\begin{align}
\frac{d^2}{d^2\gamma_{jm}} L(y_i, f_{m-1}(x_i) + \gamma_{jm})  &= \frac{d}{d\gamma_{jm}} [y_i - p(x_i)]\\
&= \frac{d}{d\gamma_{jm}} [y_i - \frac{e^{z}}{1 + e^{z}}]\\
