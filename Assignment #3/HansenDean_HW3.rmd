...
truncpolyspline <- function(x, df, natural = c(TRUE, FALSE)) {
  ...
  #truncated power basis
  if(!natural) {
  knots <- quantile(x, seq(0.25, 0.85, length = df - 4))
  trunc_fun <- function(k) (x>=k)*(x-k)^3
  s <- sapply(knots, trunc_fun)
  s <- cbind(x, x^2, x^3, s)
  return(s)
  ...
  #natural spline basis
  if(natural) {
    knots <- quantile(x, seq(0.25, 0.85, length = df))
...
Below we plot the basis functions for $df=5$.
...
```{=latex}
\begin{align}
(\xi_K-\xi_k) \sum^{K-2}_{k=1} \theta_k N_{K+2}(X) &=\sum^{K-2}_{k=1} \theta_k (\xi_K-\xi_k) [\frac{(X-\xi_k)^3_+ - (X-\xi_K)^3_+}{\xi_K - \xi_k} - \frac{(X-\xi_{K-1})^3_+ - (X-\xi_K)^3_+}{\xi_K - \xi_{K-1}}]\\
...
Thus, $\alpha_k=(\xi_K - \xi_k)\theta_k$ and we have shown the two basis representations are equivalent.
