...
We can plot more basis functions by increasing $df$.

\tiny
```{r}
xvec <- seq(0, 10, length = 101)
nat <- truncpolyspline(xvec, df = 8, natural = TRUE)
unnat <- truncpolyspline(xvec, df = 8, natural = FALSE)
par(mfrow = c(1, 2))
matplot(scale(nat), type = "l", main = "Natural")
matplot(scale(unnat), type = "l", main = "Regular")
```
...
for (i in 1:250) {  
  ...
  gcv_pred <- predict(gcv, newdata = df, type = "reponse")
...
reml_bias <- mean(reml_pred - df$z)
reml_var <- var(reml_pred - df$z)
}
...
Thus, $\alpha_{k+2}=(\xi_K - \xi_k)\theta_k$ for $k$ from $1$ to $K-2$. Therefore, we have shown the two basis are equivalent and that $f(X)= \sum^{3}_{j=0} \beta_jX^j + \sum^{K}_{k=1} \theta_k (X-\xi_k)^3_+$ can be written as $f(X)= \sum^{K}_{k=1} \alpha_k N_{k}(X)$ for above $\alpha_k$.
